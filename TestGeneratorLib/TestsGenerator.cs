...
namespace TestGeneratorLib
{
    ...
    {
        private static readonly SyntaxToken PublicModifier;//token in syntax tree
        ...
        private static ClassDeclarationSyntax GenerateClass(ClassInfo classInfo)
        {
            ...
            if (classInfo.Constructors.Count > 0)
            {
                ...
                {
                    Console.WriteLine("_" + custom.Key + " " + custom.Value);
                    variable = GenerateVariable("_" + custom.Key, $"Mock<{custom.Value}>");//varName(serega) | InterfaceName(IClonable)
        ...
        private static Dictionary<string, string> GetCustomTypeVariables(Dictionary<string, string> parameters)//structs and enums
        {
            var res = new Dictionary<string, string>();
            foreach (var parameter in parameters)
            {
        ...
        {
            return SyntaxFactory.VariableDeclaration(SyntaxFactory.ParseTypeName(typeName)).AddVariables(SyntaxFactory.VariableDeclarator(varName));
        }
